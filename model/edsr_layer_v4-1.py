...
class EDSR(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        for _ in range(n_resblock):
            m_body.append(common.ResidualBlock_noBN_v4(n_feats))
    ...
    def forward(self, x, pos_mat):
        ...
        for i in range(self.args.n_resblocks):
            ...
            #while res.shape[1]<n_feats:
                #res = torch.cat((res, res), dim=1) 
            #print(str(i+1)+' after cat res shape:'+str(res.shape))
            #res = res[:,:n_feats,:,:]
            #print(str(i+1)+' after cat2 shape:'+str(res.shape))
