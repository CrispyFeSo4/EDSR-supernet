...
class EDSR(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        # define body module 
        m_body = []
        for num in range(n_resblock):
            if(num+1)%4==0:
                m_body.append(common.ResidualBlock_noBN_v4_2(n_feats))
            else:
                m_body.append(common.ResidualBlock_noBN_v2(n_feats))
    ...
    def forward(self, x, pos_mat):
        ...
        # unet
        '''max_scale = max(self.scale,self.scale2)
        if max_scale in self.scale_list[:4]:
            width = 1.0
        elif max_scale in self.scale_list[4:7]:
            width = 0.75
        elif max_scale in self.scale_list[7:11]:
            width = 0.5
        elif max_scale in self.scale_list[11:15]:
            width = 0.25
        elif max_scale in self.scale_list[15:18]:
            width = 0.25
        elif max_scale in self.scale_list[18:22]:
            width = 0.5
        elif max_scale in self.scale_list[22:26]:
            width = 0.75
        else:
            width = 1.0'''
        ...
        for i in range(self.args.n_resblocks//4):
            for j in range(4):
                self.body[4*i+j].set_width(width)
                res = self.body[4*i+j](res)
                if j==0:
                    rrr = res
                else:
                    rrr = torch.cat((rrr, res), dim=1)
                    if j==3:
                        res = rrr[:,-self.args.n_feats:,:,:] + rrr[:,2*self.args.n_feats:3*self.args.n_feats,:,:]

    
                #print(str(4*i+j)+' layer res shape:'+str(res.shape))
