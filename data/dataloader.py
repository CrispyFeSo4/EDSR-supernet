    ...
    while True:
        ...
        try:
            ...
            if dataset.first_epoch and len(scale) > 1 and dataset.train:
                # 如果是第一个epoch scale只会是2，3，4，randidx只会从这三个里挑
                # 区分dataset有一个setscale函数，是用来调取对应尺度的lr-hr图像对；
                # model也有一个set-scale函数，而且这里set是2个scale，是设置模型参数，也就是训练尺度的
            ...
            if not dataset.first_epoch and len(scale) > 1 and dataset.train:
                idx_scale = random.randrange(0, len(scale)) # 参照原来方法
                # 但是为什么只有一个scale？
